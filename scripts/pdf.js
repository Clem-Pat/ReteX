...
function textOnclick(event){
	...
	var pageName = page.slice(0,-5)
	var list = pdfFilesDict[pageName]
	if (pageName != 'BSP') { window.open('https://retex.s3.ap-southeast-2.amazonaws.com/pdf/Retex/' + pageName + '/' + list[event.target.id],'_blank') }
	//else { window.open('../resources/pdf/BSP/' + list[event.target.id],'_blank') }
	else { window.open('https://retex.s3.ap-southeast-2.amazonaws.com/pdf/BSP/' + list[event.target.id],'_blank') }
}
