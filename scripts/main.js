...
function tab(event) {
	...
	if (path.slice(0,3) == '/C:') {
		if (page=='main.html') {window.location.assign(`./pages/${event.target.name}.html`)}
			...
			if (path.includes('RetexAsp')) {window.location.assign(`../asp.html`)}
			else if (path.includes('jeuRadio/')) {window.location.assign(`../jeuRadio.html`)}
			else if (path.includes('caserne') || path.includes('/pages/ReteX/') || path.includes('/radio/')) {window.location.assign(`../${event.target.name}.html`)}
...
function home(){
	...
	var page = path.split('/').pop();
	if (path.slice(0,3) == '/C:') {
		if (page=='main.html' || page=='index.html') {window.location.assign(`./main.html`)}
		else {
			if (path.includes('RetexAsp')) {window.location.assign(`../../../main.html`)}
			else if (path.includes('jeuRadio/')) {window.location.assign(`../../../main.html`)}
			else if (path.includes('caserne') || path.includes('/pages/ReteX/') || path.includes('/radio/')) {window.location.assign(`../../main.html`)}
...
function sendMail(){
	...
	let body = ''
	window.open(`mailto:retex@gmail.com?subject=${subject}&body=${body}`);
}
